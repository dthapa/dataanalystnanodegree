...
def plot_weather_data(turnstile_weather):
    ...
    * Which stations have more exits or entries at different times of day
    ...
    plot = ggplot(df, aes(x='ENTRIESn_hourly', color='rain')) + \
            geom_bar(binwidth = 500) + scale_x_continuous(limits=[0, 10000]) + \
            ggtitle('Entries per hour by rain conditions') + \
            xlab('Entries range (bin=500)') + ylab('Entries per hour')
